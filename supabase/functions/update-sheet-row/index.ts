...
Deno.serve(async (req) => {
  ...
  try {
    const { sheetName, rowNumber, row } = await req.json();
    const { sheetId, userId } = await getSheetIdFromRequest(req);
    const token = await getGoogleAuthToken();

    const range = `${sheetName}!A${rowNumber}:Z${rowNumber}`;
    const resp = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${range}?valueInputOption=RAW`, {
      method: "PUT",
      headers: { 
